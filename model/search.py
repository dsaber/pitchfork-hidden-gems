...
# Its primary goal is to find the best version of the following data pipeline:
    # (A) CV and TF-IDF (i.e., tuning minimum document frequency, ngram range,
    # etc.)
...
def main(scoring_func=metrics.roc_auc_score, file_path='data/final_p4k.csv',
         vocab_dict=PRODUCE_VOCAB, nlp_dict=NLP_PARAMS,
         model_names=NAMES, models=MODELS):
    # our result is going to be a dictionary where keys correspond to
    # combinations of NLP features crossed with combinations
    # of Algorithms/Associated Tuning Parameters
    result = {}

    df = pd.read_csv(file_path)
    df['Content'] = df.apply(
        lambda x: nlpp.remove(x['Artist'], x['Content']), axis=1)
    ...
    for n, p in zip(neg_kf, pos_kf):
        ...
        neg_train = pd.DataFrame(
            neg['Content'].values[neg_train_index],
            columns=['Content'])['Content']
        neg_test = pd.DataFrame(
            neg['Content'].values[neg_test_index],
            columns=['Content'])['Content']
        ...
        pos_train = pd.DataFrame(
            pos['Content'].values[pos_train_index],
            columns=['Content'])['Content']
        pos_test = pd.DataFrame(
            pos['Content'].values[pos_test_index],
            columns=['Content'])['Content']
        ...
        train_label = np.concatenate(
            [neg_train_label, pos_train_label]).ravel()
        ...
        for vocab_option in vocab_options:
            ...
            for nlp_option in nlp_options:
                ...
                if (nlp_opt['ngram_range'] != (1, 1)
                    and (voc_opt['info_thresh'] is not None
                         or nlp_opt['min_df'] == 1)):
                            pass
                else:
                    ...
                    # modeling process:
                    # (1) fit CV/TFIDF on training set;
                    # (2) transform training set with
                    # NLP processing tool from (1);
                    # (3) fit model on transformed training set
                    # from (2)
                    # (4) transform test set using NLP processing
                    # tool from (1);
                    # (5) compute scoring metric by using fitted model
                    ...
                    for model, param_dict in models.iteritems():
                        ...
                        for combo in combos:
                            pdict = {
                                k: v for k, v in zip(param_dict.keys(), combo)}
                            clf = model(**pdict)

                            # (1)-(2)
                            train_transformed = nlp_preprocessor.fit_transform(
                                train_content)
                            ...
                            # (4)
                            test_transformed = nlp_preprocessor.transform(
                                test_content)

                            # (5)
                            this_score = scoring_func(
                                test_label, clf.predict(test_transformed))

                            # Keep track of information in result
                            this_to_str = (str(vocab_option) +
                                           str(nlp_option) +
                                           model_names[model] + str(combo))
...
if __name__ == '__main__':
    result = main()
    cPickle.dump(
        result, open('model/grid_search/grid_search_complete.pkl', 'w'))
