...
def get_genre_data(c_id, u_id, artist_name, album_name):
    metadata = pygn.search(
        clientID=c_id, userID=u_id, artist=artist_name, album=album_name)
...
if __name__ == '__main__':
    ...
    # use 'for' loop to avoid slamming Gracenote
    # with 10's of 1000's of requests concurrently
    for i in xrange(df.shape[0]):
        print i
        try:
            album_genre_id, album_genre_name = get_genre_data(
                CLIENT_ID, USER_ID, df['Artist'][i], df['Album'][i])
