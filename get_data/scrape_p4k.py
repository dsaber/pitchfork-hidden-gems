...
def scrape_pitchfork(url_root='http://pitchfork.com/reviews/albums/',
                     begin_page=1, end_page=780):
    ...
    for page in xrange(begin_page, end_page + 1):
        ...
        for album, link in zip(rev_info, rev_links):
            # this try/except probably isn't wonderful form;
            # however, I intensely validated the code in the "try" block
            # the try/except is to avoid bad HTML that seems to crops up
            # randomly
            try:
                # the Artist/Album/Reviewer/Date fields
                # should be self-explanatory
                result['Artist'].append(album.h1.text.encode('ascii',
                                                             'ignore'))
                result['Album'].append(album.h2.text.encode('ascii',
                                                            'ignore'))
                result['Reviewer'].append(album.h3.text.encode('ascii',
                                                               'ignore')[3:])
                ...
                # get actual review content, along with
                # score and whether or not the
                ...
                # (need the try/excepts for a couple of
                # instances of mal-formed data)
                try:
                    result['Artwork'].append(rev_content.img['src'])
                except:
                    result['Artwork'].append('')
                try:
                    result['Score'].append(float(rev_content.select(
                        '.score')[0].text.encode('ascii', 'ignore')))
                except:
                ...
                # Review Content
                result['Content'].append(rev_content.select(
                    '.editorial')[0].text.encode('ascii', 'ignore'))
            except:
